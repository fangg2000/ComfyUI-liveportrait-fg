...
class LivePortraitPipeline(object):
    ...
    def execute(self, args: ArgumentConfig):
        ...
        if flag_load_from_template:
            ...
            if args.driving_name != '':
                wfp_template_name = (remove_suffix(args.driving) if args.driving_name is None else args.driving_name) + '_fg.pkl'
                # log(f'模板目录--{args.root_path}')
                wfp_template = args.dir_path + args.driving_path +'/'+ wfp_template_name
            else:
                wfp_template = remove_suffix(args.driving) + '.pkl'

            # log(f'模板--{wfp_template}')
